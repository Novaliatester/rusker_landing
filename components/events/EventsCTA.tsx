...
export default function EventsCTA() {
  ...
  return (
    <section id="events-cta" ref={ref} className="relative py-20 md:py-32 bg-white overflow-hidden">
      {/* Background decoration */}
      <div className="absolute inset-0">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-rusker-events/20 to-transparent" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,_rgba(11,93,86,0.08),_transparent_50%)]" />
      </div>
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="group relative px-10 py-5 bg-rusker-events text-white font-bold text-lg rounded-full overflow-hidden shadow-xl shadow-rusker-events/30"
            >
              {/* Animated background */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-[#094a44] via-rusker-events to-[#094a44]"
                animate={{
                  x: isHovered ? ['0%', '100%'] : '0%',
                }}
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-md">
              <div className="flex gap-2 p-2 bg-gray-50 rounded-full border border-gray-200 focus-within:border-rusker-events focus-within:ring-2 focus-within:ring-rusker-events/20 transition-all">
                <input
                  type="email"
                  value={email}
